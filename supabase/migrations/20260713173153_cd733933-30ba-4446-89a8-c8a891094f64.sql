...
-- 1. Add sizes column to menu_items (array of {name, price})
ALTER TABLE public.menu_items
  ADD COLUMN IF NOT EXISTS sizes jsonb;

-- 2. Update place_order to support size selection per item
CREATE OR REPLACE FUNCTION public.place_order(
  _restaurant_id uuid,
  _delivery_address text,
  _delivery_latitude numeric,
  _delivery_longitude numeric,
  _special_instructions text,
  _payment_method text,
  _items jsonb,
  _estimated_minutes integer
)
...
DECLARE
  ...
  _size_name text; _size_price numeric; _unit_price numeric;
  _display_name text; _size_obj jsonb; _found boolean;
BEGIN
  ...
  FOR _item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    SELECT mi.id, mi.name, mi.price, mi.restaurant_id, mi.is_available, mi.sizes INTO _menu
    ...
    _size_name := NULLIF(TRIM(COALESCE(_item->>'size','')), '');
    _unit_price := _menu.price;

    IF _menu.sizes IS NOT NULL AND jsonb_array_length(_menu.sizes) > 0 THEN
      IF _size_name IS NULL THEN
        RAISE EXCEPTION 'Please select a size for %', _menu.name;
      END IF;
      _found := false;
      FOR _size_obj IN SELECT * FROM jsonb_array_elements(_menu.sizes) LOOP
        IF LOWER(TRIM(_size_obj->>'name')) = LOWER(_size_name) THEN
          _unit_price := (_size_obj->>'price')::numeric;
          _size_name := _size_obj->>'name';
          _found := true;
          EXIT;
        END IF;
      END LOOP;
      IF NOT _found THEN RAISE EXCEPTION 'Invalid size for %', _menu.name; END IF;
    ELSE
      _size_name := NULL;
    END IF;
  ...
  -- Insert order items with resolved size/price
  FOR _item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    _qty := (_item->>'quantity')::int;
    SELECT mi.id, mi.name, mi.price, mi.sizes INTO _menu
      FROM public.menu_items mi WHERE mi.id = (_item->>'menu_item_id')::uuid;

    _size_name := NULLIF(TRIM(COALESCE(_item->>'size','')), '');
    _unit_price := _menu.price;
    _display_name := _menu.name;

    IF _menu.sizes IS NOT NULL AND jsonb_array_length(_menu.sizes) > 0 AND _size_name IS NOT NULL THEN
      FOR _size_obj IN SELECT * FROM jsonb_array_elements(_menu.sizes) LOOP
        IF LOWER(TRIM(_size_obj->>'name')) = LOWER(_size_name) THEN
          _unit_price := (_size_obj->>'price')::numeric;
          _display_name := _menu.name || ' (' || (_size_obj->>'name') || ')';
          EXIT;
        END IF;
      END LOOP;
    END IF;
    ...
    INSERT INTO public.order_items (
      ...
      _new_id, _menu.id, _display_name, _unit_price,
      _qty, _unit_price * _qty,
